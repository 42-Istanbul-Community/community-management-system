...
export RUSTFS_ACCESS_KEY="$(cat /run/secrets/rustfs_content_access_key)"
export RUSTFS_SECRET_KEY="$(cat /run/secrets/rustfs_content_secret_key)"

if [ -n "$DB_PASSWORD_FILE" ]; then
    DB_PASSWORD=$(cat "$DB_PASSWORD_FILE")
fi

export DATABASE_URL="postgresql://${POSTGRES_USER}:${DB_PASSWORD}@postgres-content:5432/${POSTGRES_DB}?schema=public"
export PSQL_URL="postgresql://${POSTGRES_USER}:${DB_PASSWORD}@postgres-content:5432/${POSTGRES_DB}"
